...
echo "<table border='1'>";
echo "<tr><td> Student ID</td> <td> Time Slots</td><td> Email</td><td> Date</td><td> First Name</td><td>Last Name</td><td>Expiry</td></tr>";
while ($row =mysqli_fetch_assoc($result)) {
echo "<tr><td> {$row['student_id']}</td> <td> {$row['timeslot']}</td><td> {$row['email']} </td><td> {$row['date']}</td><td> {$row['first_name']}</td>
<td> {$row['last_name']}</td><td><a href='expired.php?student_id=$row[student_id]'>expired</a></td></tr>";
}
echo"</table>";
